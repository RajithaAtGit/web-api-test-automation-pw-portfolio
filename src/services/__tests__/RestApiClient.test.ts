...
import { ApiRequestOptions, ApiResponse, HttpMethod } from '../../interfaces/IApiClient';
...
const createMockAPIResponse = (overrides: Partial<APIResponse> = {}): jest.Mocked<APIResponse> => {
  return {
...
describe('RestApiClient', () => {
  let mockRequestContext: jest.Mocked<APIRequestContext>;
  let apiClient: RestApiClient;

  beforeEach(() => {
    mockRequestContext = createMockAPIRequestContext();
    apiClient = new RestApiClient(mockRequestContext);
  });
  ...
  describe('constructor', () => {
    it('should initialize with the provided request context and base URL', () => {
      ...
      const client = new RestApiClient(mockRequestContext, baseUrl);
    ...
    it('should use the ConfigManager to get the base URL if not provided', () => {
      ...
      expect(ConfigManager.getInstance().getApiBaseUrl).toHaveBeenCalled();
    });
  ...
  describe('create', () => {
    it('should create a new RestApiClient instance', async () => {
      const client = await RestApiClient.create();
      expect(client).toBeInstanceOf(RestApiClient);
    ...
    it('should use the provided base URL', async () => {
      const baseUrl = 'https://custom.example.com';
      const client = await RestApiClient.create(baseUrl);
      expect(client['baseUrl']).toBe(baseUrl);
      expect(playwrightRequest.newContext).toHaveBeenCalledWith({
        baseURL: baseUrl,
        ignoreHTTPSErrors: true
  ...
  describe('clone', () => {
    it('should create a new instance with the same configuration', () => {
      ...
      expect(clone).toBeInstanceOf(RestApiClient);
